...
const AllBaseServices = ({
  ...
  console.log("Base Services data", datas);

  const { onDownload } = useDownloadExcel({
    currentTableRef: tableRef.current,
    filename: "Base_Service_List",
    sheet: "Base Services",
  });

  const exportToPDF = () => {
    const doc = new jsPDF();
    doc.text("Base Service List", 14, 10);
  ...
  const handleView = (service) => {
    navigate(`/base-additional-services/view/${service._id}`, {
      state: { serviceData: service },
    });
  };

  const handleEdit = (service) => {
    navigate(`/base-additional-services/edit/${service._id}`, {
      state: { serviceData: service },
    });
  };
    ...
    try {
      ...
      if (result.isConfirmed) {
        const response = await axios.delete(
          `https://api.mrbikedoctor.cloud/bikedoctor/additional-service/deleteBaseAdditionalService/${data._id}`
        );
...
export default AllBaseServices;
