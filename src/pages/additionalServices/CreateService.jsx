...
const CreateAddService = () => {
  ...
  // Bike Pricing States
  const [bikes, setBikes] = useState([]); // Array of { id, cc, price }
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const [dealerResponse, baseServiceResponse] = await Promise.all([
          getDealerList(),
          getBaseAdditionalServices(),
        ]);
  ...
  // For Additional Services, the rules actually "Generate" more rows if they don't exist
  // Or we just allow them to apply to existing rows.
  // Given the original UI was manual CC entry, let's treat Rules as "Batch Add CC Groups"
  // and DataGrid for manual entry.

  const applyRules = () => {
    if (pricingRules.length === 0) return;

    // We'll generate a list of CCs from rules.
    // For simplicity in Additional Services, we'll just treat each rule as one row entry
    // reflecting that range, or allow the admin to manually add specific CCs.
    // However, since the API expects { cc, price }, we better keep it individual.

    // Logic: Each rule creates a entry for the CCs in that range if specified,
    // or just use the rule as a "Dynamic Price" for anything matching.
    // Let's stick to the same "Computed" vs "Manual" pattern as main Services for consistency.
  };

  // We need a list of CCs to display in the grid.
  // In Additional services, the user manually added rows.
  // We'll provide a "Manual Add" row button, and Rules will auto-price them.
  const addManualCcRow = () => {
    const nextId =
      bikes.length > 0 ? Math.max(...bikes.map((b) => b.id)) + 1 : 1;
    ...
    const updatedBikes = bikes.map((b) =>
      b.id === newRow.id
        ? {
            ...
            cc: newRow.cc,
            manualPrice:
              newRow.effectivePrice === null || newRow.effectivePrice === ""
                ? null
                : Number(newRow.effectivePrice),
          }
        : b,
    );
  ...
  const validate = () => {
    ...
    const pricedBikes = resolvedBikes.filter(
      (b) => b.cc && b.effectivePrice > 0,
    );
    if (pricedBikes.length === 0)
      errors.bikes = "At least one bike CC & Price is required";
    ...
    try {
      ...
      setAlertInfo({
        ...
        message:
          ...
          "Failed to create additional service",
        severity: "error",
      });
    } finally {
      setLoading(false);
                  ...
                  <Card variant="outlined" sx={{ mb: 3, bgcolor: "grey.50" }}>
                    <CardContent>
                      ...
                      <Typography
                        ...
                        sx={{ mb: 2 }}
                      >
                        Define rules to automatically apply prices to your CC
                        entries below.
                      </Typography>
                      ...
                      <Stack
                        direction={{ xs: "column", sm: "row" }}
                        spacing={2}
                        sx={{ mb: pricingRules.length > 0 ? 3 : 0 }}
                      >
                        <TextField
                          size="small"
                          label="Min CC"
                          ...
                          {pricingRules.map((rule) => (
                            ...
                            >
                              <Typography
                                ...
                                sx={{ flexGrow: 1, ml: 1 }}
                              >
                                between <b>{rule.minCc}</b> and{" "}
                                <b>{rule.maxCc} CC</b> = <b>₹{rule.price}</b>
                              </Typography>
                              <IconButton
                                size="small"
                                color="error"
